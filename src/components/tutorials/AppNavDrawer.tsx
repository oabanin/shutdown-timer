...
import LogoWithCopyMenu from "@/components/tutorials/LogoWithCopyMenu";
import AppNavDrawerItem from "@/components/tutorials/AppNavDrawerItem";
import { pageToTitleI18n } from "@/components/tutorials/helpers";
import PageContext from "@/components/tutorials/PageContext";
import MuiProductSelector from "@/components/tutorials/MuiProductSelector";
...
function PersistScroll(props) {
  ...
  useEnhancedEffect(() => {
    ...
    const activeDrawerLink =
      scrollContainer.querySelector(".app-drawer-active");
...
function reduceChildRoutes(context) {
  ...
  if (page.children && page.children.length >= 1) {
    const topLevel = activePageParents.some(
            ...
            {versions.map((item) => {
              ...
              return (
                ...
                >
                  {item.text}{" "}
                  {item.current && (
                    <DoneRounded sx={{ fontSize: 16, ml: "auto" }} />
                  )}
                </MenuItem>
              );
            })}
